...
exports.getPoiGeoJson = async (req, res) => {
  try {
    const { category, min_score, feature_group, search, min_lng, min_lat, max_lng, max_lat } = req.query;
    ...
    if (feature_group) {
      whereClause += ` AND EXISTS (
        ...
        WHERE pa.poi_id = p.id AND af.feature_group = $${idx} AND pa.is_available = TRUE
      )`;
    ...
    if (min_lng && min_lat && max_lng && max_lat) {
      ...
      params.push(parseFloat(min_lng), parseFloat(min_lat), parseFloat(max_lng), parseFloat(max_lat));
      idx += 4;
    }
...
exports.createPoi = async (req, res) => {
  ...
  try {
    ...
    const {
      name, name_vi, description, address,
      lat, lng,
      category_id, phone, website, opening_hours,
    ...
    if (!name || lat == null || lng == null) {
      return res.status(400).json({ status: 'error', message: 'name, lat, lng are required' });
    }

    const poiStatus = (req.user.role === 'admin' || req.user.role === 'moderator') ? 'approved' : 'pending';
    ...
    `, [name, name_vi, description, address, parseFloat(lng), parseFloat(lat),
        category_id, phone, website, opening_hours, user_id, poiStatus]);
    ...
    if (accessibility_features && accessibility_features.length > 0) {
      for (const feat of accessibility_features) {
        if (feat.quality_rating && (feat.quality_rating < 1 || feat.quality_rating > 10)) {
          await client.query('ROLLBACK');
          return res.status(400).json({ status: 'error', message: 'quality_rating must be 1-10' });
        }
        await client.query(`
          INSERT INTO poi_accessibility (poi_id, feature_id, is_available, quality_rating, note, reported_by)
          VALUES ($1, $2, $3, $4, $5, $6)
        `, [poiId, feat.feature_id, feat.is_available ?? true, feat.quality_rating, feat.note, user_id]);
      }
...
exports.updatePoi = async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const poiCheck = await client.query('SELECT * FROM pois WHERE id = $1 AND deleted_at IS NULL', [parseInt(req.params.id)]);
    ...
    if (String(oldPoi.user_id) !== String(req.user.id) && req.user.role !== 'admin' && req.user.role !== 'moderator') {
      await client.query('ROLLBACK');
      return res.status(403).json({ status: 'error', message: 'Forbidden: You do not own this POI' });
    }
    ...
    `, [name, name_vi, description, address,
        lng ? parseFloat(lng) : null, lat ? parseFloat(lat) : null,
        category_id, phone, website, opening_hours, parseInt(req.params.id)]);
...
exports.deletePoi = async (req, res) => {
  ...
  try {
    ...
    const result = await client.query('UPDATE pois SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id', [parseInt(req.params.id)]);
    await client.query('COMMIT');
    res.json({ status: 'success', message: 'POI deleted (soft)', data: { id: result.rows[0].id } });
  } catch (err) {
    await client.query('ROLLBACK');
    res.status(500).json({ status: 'error', message: err.message });
  } finally {
...
exports.rollbackPoi = async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    const historyCheck = await client.query('SELECT * FROM edit_history WHERE id = $1', [parseInt(req.params.history_id)]);
    ...
    if (record.action_type === 'delete') {
      ...
      await client.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          updated_at = CURRENT_TIMESTAMP
      `, [data.id, data.name, data.name_vi, data.description, data.address, data.location, data.overall_score, data.is_verified, data.category_id, data.phone, data.website, data.opening_hours, data.user_id, data.created_at, data.updated_at]);
    } else {
      await client.query(`
        UPDATE pois SET
          ...
          updated_at = CURRENT_TIMESTAMP
        WHERE id = $10
      `, [data.name, data.name_vi, data.description, data.address, data.location, data.category_id, data.phone, data.website, data.opening_hours, data.id]);
    }
    
    await client.query('DELETE FROM edit_history WHERE id = $1', [parseInt(req.params.history_id)]);
    await client.query('COMMIT');
    res.json({ status: 'success', message: 'Rollback successful', data: { poi_id: record.poi_id } });
  } catch (err) {
    await client.query('ROLLBACK');
    res.status(500).json({ status: 'error', message: err.message });
  } finally {
...
exports.flagPoi = async (req, res) => {
  ...
  try {
    ...
    const flagCheck = await client.query('SELECT id FROM poi_flags WHERE poi_id = $1 AND user_id = $2', [poiId, userId]);
    if (flagCheck.rows.length > 0) {
      await client.query('ROLLBACK');
      return res.status(400).json({ status: 'error', message: 'You have already flagged this location' });
    }

    await client.query('INSERT INTO poi_flags (poi_id, user_id, reason) VALUES ($1, $2, $3)', [poiId, userId, reason]);
    ...
    await client.query('COMMIT');
    res.json({ status: 'success', message: 'POI flagged successfully', data: updateRes.rows[0] });
  } catch (err) {
    await client.query('ROLLBACK');
    res.status(500).json({ status: 'error', message: err.message });
  } finally {
